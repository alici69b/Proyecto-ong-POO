...
// Segun el tipo que recojamos, se guardata uno u otro tipo
...
if (!empty($err_email))     $errores['email']      = $err_email;
if (!empty($err_password))  $errores['password']   = $err_password;
if (!empty($err_tipoayuda)) $errores['tipo_ayuda'] = $err_tipoayuda;

// 4. Si hay errores, redirigimos con ellos en sesión
if (!empty($errores)) {
...
// recogemos los datos y los guardamos en un array para luego utilizarlos 
$datos = [
    'nombre'          => $nombre,
    'apellidos'       => $apellidos,
    'email'           => $email,
