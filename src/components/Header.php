...
if ($rol === 'admin') {
    $dashboard_url = BASE_URL . '/app/controllers/controller_admin_dashboard.php';
} elseif ($rol === 'soy-voluntario') {
    $dashboard_url = BASE_URL . '/app/controllers/controller_volunteer_dashboard.php';
} else {
    $dashboard_url = BASE_URL . '/app/controllers/controller_user_dashboard.php';
}
?>
<nav class="absolute top-6 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl bg-white/60 backdrop-blur-md border border-white/10 shadow-lg rounded-full z-[100] px-6 py-3 flex items-center justify-between">
    ...
    <div class="flex-1 flex justify-start">
        <a href="<?= BASE_URL ?>/index.php" class="flex items-center gap-2 hover:opacity-80 transition group">
    ...
    <div class="hidden lg:flex flex-none items-center justify-center gap-6">
        <a class="text-gray-600 hover:text-[#25a18e] font-medium transition" href="<?= BASE_URL ?>/index.php">Inicio</a>
    ...
    <div class="flex-1 flex justify-end items-center gap-3">
        <div class="hidden lg:flex items-center gap-3">
            ...
            <a href="<?= $dashboard_url ?>" class="flex items-center gap-2 text-sm font-bold text-[#004e64] hover:text-[#00a5cf] transition mr-1 bg-white/60 px-2 py-1 rounded-full backdrop-blur-sm">
                <?php if (!empty($fotoPerfil)): ?>
                    <img src="<?= BASE_URL ?>/public/img/<?= htmlspecialchars($fotoPerfil) ?>" class="w-7 h-7 rounded-full object-cover">
                <?php else: ?>
            ...
            <a class="px-5 py-2 bg-[#25a18e] text-white rounded-full hover:bg-[#1a7a6b] transition font-bold text-sm shadow-md" href="<?= BASE_URL ?>/app/controllers/controller_logout.php">Cerrar Sesión</a>
            <?php else: ?>
            <a class="px-5 py-2 border-2 border-[#25a18e] text-[#25a18e] rounded-full hover:bg-[#25a18e] hover:text-white transition font-bold text-sm" href="<?= BASE_URL ?>/app/controllers/controller_login.php">Iniciar Sesión</a>
            <a class="px-5 py-2 bg-[#25a18e] text-white rounded-full hover:bg-[#1a7a6b] transition font-bold text-sm shadow-md" href="<?= BASE_URL ?>/app/controllers/controller_register.php">Registro</a>
            <?php endif; ?>
        </div>
        ...
        <div class="lg:hidden flex items-center">
            ...
            <div class="absolute top-full left-0 right-0 mt-4 mx-2 bg-white rounded-2xl shadow-2xl border border-gray-100 flex-col hidden peer-checked:flex overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300 z-50">
                <a class="px-6 py-4 hover:bg-gray-50 text-gray-700 border-b border-gray-50" href="<?= BASE_URL ?>/index.php">Inicio</a>
                ...
                <div class="bg-gray-50 flex flex-col gap-1 p-4">
                    <?php if ($loggedIn): ?>
                    <a class="w-full py-3 text-center border-2 border-[#00a5cf] text-[#00a5cf] rounded-xl font-bold" href="<?= $dashboard_url ?>">Mi Perfil</a>
                    <a class="w-full py-3 text-center bg-[#25a18e] text-white rounded-xl font-bold" href="<?= BASE_URL ?>/app/controllers/controller_logout.php">Cerrar Sesión</a>
                    <?php else: ?>
                    <a class="w-full py-3 text-center border-2 border-[#25a18e] text-[#25a18e] rounded-xl font-bold" href="<?= BASE_URL ?>/app/controllers/controller_login.php">Iniciar Sesión</a>
                    <a class="w-full py-3 text-center bg-[#25a18e] text-white rounded-xl font-bold" href="<?= BASE_URL ?>/app/controllers/controller_register.php">Registro</a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
